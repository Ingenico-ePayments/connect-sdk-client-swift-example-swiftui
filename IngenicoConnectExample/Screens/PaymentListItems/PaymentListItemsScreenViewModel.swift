...
extension PaymentItemListScreen {
    ...
    class ViewModel: NSObject, ObservableObject, PKPaymentAuthorizationViewControllerDelegate {
        ...
        @Published var showPaymentProduct: Bool = false
        ...
        func didSelect(item: PaymentProductsRow, accountOnFile: Bool) {
            ...
            if paymentItem is BasicPaymentProduct, let session = session, let context = context {
                session.paymentProduct(withId: paymentItem.identifier, context: context) { [weak self] paymentProduct in
                    if paymentItem.identifier.isEqual(SDKConstants.kApplePayIdentifier) {
                        ...
                        if paymentProduct.fields.paymentProductFields.count > 0 {
                            ...
                            if accountOnFile {
                                tempAccountOnFile = paymentProduct.accountOnFile(withIdentifier: item.accountOnFileIdentifier)
                ...
                session.paymentProductGroup(withId: paymentItem.identifier, context: context) { [weak self] paymentProductGroup in
                    self?.isLoading = false
                    self?.selectedAccountOnFile = nil
        ...
        func prepareItems(paymentItems: PaymentItems?) {
            ...
            if hasAccountsOnFile {
                self.accountsOnFile = generateRowsFrom(accountsOnFile: paymentItems.accountsOnFile, paymentItems: paymentItems)
            }
        ...
        private func generateRowsFrom(accountsOnFile: [AccountOnFile], paymentItems: PaymentItems) -> [PaymentProductsRow] {
            var items: [PaymentProductsRow] = []
            
            for accountOnFile in accountsOnFile.sorted(by: { (a, b) -> Bool in
                paymentItems.paymentItem(withIdentifier: a.paymentProductIdentifier)?.displayHints.displayOrder ?? Int.max < paymentItems.paymentItem(withIdentifier: b.paymentProductIdentifier)?.displayHints.displayOrder ?? Int.max
            }) {
        ...
        func showApplePayPaymentItem(paymentProduct: PaymentProduct?) {
            if SDKConstants.SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(v: "13.0") && PKPaymentAuthorizationViewController.canMakePayments() {
        ...
        func showApplePaySheet(for paymentProduct: PaymentProduct, context: PaymentContext, withAvailableNetworks paymentNetworks: [PKPaymentNetwork]) {
            ...
            if let authorizationViewController = authorizationViewController, PKPaymentAuthorizationViewController.canMakePayments(usingNetworks: paymentNetworks) {
                applePayPaymentProduct = paymentProduct
                UIApplication.shared.rootViewController?.present(authorizationViewController, animated: true, completion: nil)
            }
        }
