                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ProductTextFieldView(leadingImage: Image(systemName: "creditcard"),
                                             trailingImage: .image(Image(uiImage: viewModel.paymentItem?.displayHints.logoImage ?? UIImage())),
                                             placeholder: viewModel.placeholder(forField: viewModel.creditCardField) ?? "",
                                             text: viewModel.creditCardFieldEnabled ?
                                             Binding<String>(
                                                get: { viewModel.maskedValue(forField: viewModel.creditCardField) },
                                                set: { viewModel.setValue(value: $0, forField: viewModel.creditCardField);
                                                    viewModel.didChangeCreditCardField($0) }) :
                                .constant(viewModel.accountOnFile?.label ?? ""),
                                             accentColor: viewModel.creditCardFieldEnabled ? Constants.accentColor : Constants.inactiveColor,
                                             borderColor: viewModel.creditCardFieldEnabled ? Constants.borderColor : Constants.inactiveColor,
                        ...
                        HStack(alignment: .top) {
                            ProductTextFieldView(leadingImage: Image(systemName: "calendar"),
                                                 trailingImage: .none,
                                                 placeholder: viewModel.placeholder(forField: viewModel.expiryDateField) ?? "",
                                                 text:  Binding<String>(
                                                    get: { viewModel.maskedValue(forField: viewModel.expiryDateField) },
                                                    set: { viewModel.setValue(value: $0, forField: viewModel.expiryDateField);
                                                                              viewModel.didChangeExpiryDateField($0) }),
                                                 accentColor: viewModel.creditCardFieldEnabled ? Constants.accentColor : Constants.inactiveColor,
                                                 borderColor: viewModel.creditCardFieldEnabled ? Constants.borderColor : Constants.inactiveColor,
                        ...
                        ProductTextFieldView(leadingImage: Image(systemName: "person.fill"),
                                             trailingImage: .none,
                                             placeholder: viewModel.placeholder(forField: viewModel.cardHolderField) ?? "",
                                             text: Binding<String>(
                                                get: { viewModel.maskedValue(forField: viewModel.cardHolderField) },
                                                set: { viewModel.setValue(value: $0, forField: viewModel.cardHolderField);
                                                    viewModel.didChangeCardHolder($0)
                                                }),
                                             accentColor: viewModel.creditCardFieldEnabled ? Constants.accentColor : Constants.inactiveColor,
                                             borderColor: viewModel.creditCardFieldEnabled ? Constants.borderColor : Constants.inactiveColor,
                    ...
                    NavigationLink("", isActive: $viewModel.showEndScreen) {
                        EndScreen(viewModel: EndScreen.ViewModel(preparedPaymentRequest: self.viewModel.preparedPaymentRequest))
                    }
                }
            }
...
struct CardProductScreen_Previews: PreviewProvider {
    static var previews: some View {
        CardProductScreen(viewModel: CardProductScreen.ViewModel(paymentItem: nil, session: nil, context: nil, accountOnFile: nil))
    }
}
